...
NUM_OF_ELEMENTS = 32  # if not equal to NUM_OF_ELEMENTS, needs interpolation for DeepONet branch input
...
MODES_OF_INTEREST = [1, 3, 5] # list(range(1, 102, 10))  # [1, 31, 61]
...
PLOT_INTERVAL = 500
PRINT_INTERVAL = 100
LOSS_FUNC = 'L2_relative'
if PROBLEM == 'helmholtz' and DIMENSIONS == 1:
    LOSS_FUNC = 'L2_between'

# MODEL_NAME = 'training_' + str(datetime.now())[:10] + '_l_shaped'
if DIMENSIONS == 1:
    MODEL_NAME = 'training_2022-12-02_helmholtz_1d'
    DATASET_NAME = 'processed_data_helmholtz_1d.pkl'
if DIMENSIONS == 2:
    if L_SHAPED:
